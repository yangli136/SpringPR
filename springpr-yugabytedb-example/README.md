...
```
export {{http,https,ftp}_proxy,{HTTP,HTTPS,FTP}_PROXY}="http://proxy.springpr.dev:8080"
export {NO_PROXY,no_proxy}=".americanexpress.com,.springpr.dev,localhost,127.0.0.1"
...
```
python3 -m pip config set --user global.index_url 'https://artifactory.springpr.dev/api/pypi/pypi/simple'
python3 -m pip config set --user global.index 'https://artifactory.springpr.dev/api/pypi/pypi/simple'
```
...
```
mvn -P'local-yugabyte,!lib' spring-boot:run --define spring-boot.run.arguments="--spring.profiles.active=e0,jpa,local-yugabyte"
```
